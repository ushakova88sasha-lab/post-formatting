...
Поле `buttons` необязательно. До 10 кнопок; у каждой кнопки обязательны текст и URL (`http://` или `https://`).
...
- `click_count` — число **уникальных** переходов по кнопке (один посетитель = один клик).
